 ...
 */
class Materiel implements IRessource {
...
';
    ...
    public function getName() {
        ...
        else if($this->Id_ressource == 'LIC') {
            return 'Licence';
        }
        else if($this->Id_ressource == 'LOG') {
            return 'Logiciel';
        }
        else if($this->Id_ressource == 'TJM') {
            return 'Ressource TJM';
        }
    }
    ...
    public static function getAgency($i) {
        if (self::getCegidEmployeeCode($i) == '') {
            ...
            $db = connecter_cegid();
            return $db->query('SELECT PSA_ETABLISSEMENT FROM SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($i) . '"
		    UNION SELECT PSA_ETABLISSEMENT FROM ALCYON.DBO.SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($i) . '"
		     UNION SELECT PSA_ETABLISSEMENT FROM ALTIQUENET.DBO.SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($i) . '"
		     UNION SELECT PSA_ETABLISSEMENT FROM ELITEX.DBO.SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($i) . '"
			 UNION SELECT PSA_ETABLISSEMENT FROM LYNT.DBO.SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($i) . '"')->fetchRow()->psa_etablissement;
        }
    }
