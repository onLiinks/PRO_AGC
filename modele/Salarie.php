 ...
 */
class Salarie implements IRessource {
            ...
            elseif ($code && empty($tab)) {
                ...
                foreach ($_SESSION['cegid_databases'] as $cegid_database) {
                    ...
                    $requete .= ' SELECT "' . $cegid_database . '" AS SOCIETE, PSE_EMAILPROF,ARS_AUXILIAIRE,ARS_RESSOURCE,ARS_TYPERESSOURCE,ARS_LIBELLE,ARS_LIBELLE2,ARS_ADRESSE1,ARS_ADRESSE2,
                                    ARS_ADRESSE3,ARS_CODEPOSTAL,ARS_VILLE,ARS_TELEPHONE,ARS_TELEPHONE2,PSA_LIBELLE,PSA_PRENOM,PSA_NOMJF,PSA_NUMEROSS, PSA_DATESORTIE,
                                    ...
                                    , PSE_CODESERVICE,CC2.CC_LIBELLE,CC3.CC_LIBELLE AS cc3_libelle,CC5.CC_LIBELLE AS POLE,CO3.CO_LIBELLE,PY4.PY_NATIONALITE, PSA_DATESORTIE
                ...
                if ($this->type_ressource == 'SAL') {
                    ...
                    $this->nom_ressource = $ligne->psa_libelle;
                    $this->prenom_ressource = $ligne->psa_prenom;
                    if ($ligne->pse_emailprof == 'sophie.lichten@proservia.fr') {
                        $this->prenom = 'Sophie';
                    }
                    ...
                    $this->date_sortie = str_replace(' 00:00:00', '', $ligne->psa_datesortie);
                    $this->salaire = ((float) ($ligne->psa_salairemois1) * 12) / 1000;

                    if ($ligne->psa_indice == '0001' || $ligne->psa_indice == '0002' || $ligne->psa_indice == '0005' || $ligne->psa_indice == '0007') {
                        $this->statut = 'CADRE';
                    } elseif ($ligne->psa_indice == '0003' || $ligne->psa_indice == '0004' || $ligne->psa_indice == '0006' || $ligne->psa_indice == '0008') {
                        $this->statut = 'ETAM';
                    }
                    $this->type_embauche = self::getContractType($this->Id_ressource);
                    $this->fin_cdd = str_replace(' 00:00:00', '', $ligne->psa_datesortie);
    ...
    public function getList($type = null) {
        return $this->getEmployeeList() . $this->getExEmployeeList();
    }
    ...
    public function getEmployeeList() {
        ...
        foreach ($_SESSION['cegid_databases'] as $cegid_database) {
            $requete .= ($i != 0) ? 'UNION' : '';
            $requete .= ' SELECT ARS_RESSOURCE, PSA_LIBELLE, PSA_PRENOM, PSA_CODEPOSTAL, PSA_CODESTAT, PSE_EMAILPROF FROM ' . $cegid_database . '.DBO.RESSOURCE
            INNER JOIN ' . $cegid_database . '.DBO.SALARIES AS SALARIES ON ARS_SALARIE=SALARIES.PSA_SALARIE 
            INNER JOIN ' . $cegid_database . '.DBO.DEPORTSAL ON PSE_SALARIE = PSA_SALARIE
            WHERE (ARS_FERME<>"X" AND ARS_SALARIE <> "" ) AND (PSA_DATESORTIE >= CONVERT(varchar(8), GETDATE(), 112) OR PSA_DATESORTIE = "")';
            $i++;
        }
        ...
        while ($ligne = $result->fetchRow()) {
            $html .= '<option class="grisf" value="' . $ligne->ars_ressource . '" ' . $ressource[$ligne->ars_ressource] . '>' . substr($ligne->psa_libelle, 0, 10) . ' ' . substr($ligne->psa_prenom, 0, 10) . ' | ' . $ligne->psa_codepostal . ' | ' . $ligne->pse_emailprof . '</option>';
        }
        return $html;
    }
    ...
    public function getExEmployeeList() {
        ...
        foreach ($_SESSION['cegid_databases'] as $cegid_database) {
            ...
            INNER JOIN ' . $cegid_database . '.DBO.SALARIES AS SALARIES ON ARS_SALARIE=SALARIES.PSA_SALARIE 
            INNER JOIN ' . $cegid_database . '.DBO.DEPORTSAL ON PSE_SALARIE = PSA_SALARIE
            WHERE (ARS_FERME<>"X" AND ARS_SALARIE <> "" ) AND (PSA_DATESORTIE < GETDATE() AND PSA_DATESORTIE != "")';
            $i++;
        }
        $requete .= ' ORDER BY PSA_LIBELLE';
        $result = $db->query($requete);
        while ($ligne = $result->fetchRow()) {
            $html .= '<option class="roworange" value="' . $ligne->ars_ressource . '" ' . $ressource[$ligne->ars_ressource] . '>' . substr($ligne->psa_libelle, 0, 10) . ' ' . substr($ligne->psa_prenom, 0, 10) . ' | ' . $ligne->psa_codepostal . ' | ' . $ligne->pse_emailprof . '</option>';
        }
        return $html;
    }
...
';
    ...
    public function getName() {
        ...
        foreach ($_SESSION['cegid_databases'] as $cegid_database) {
            $requete .= ($j != 0) ? ' UNION' : '';
            $requete .= ' SELECT ARS_TYPERESSOURCE,ARS_RESSOURCE,ARS_LIBELLE,ARS_LIBELLE2,PSA_LIBELLE,PSA_PRENOM
                          FROM ' . $cegid_database . '.DBO.RESSOURCE 
                          LEFT JOIN ' . $cegid_database . '.DBO.SALARIES AS salaries ON ars_salarie=salaries.psa_salarie 
                          WHERE ARS_RESSOURCE="' . $this->Id_ressource . '"';
            $j++;
        }
    ...
    public static function getMail($i) {
        if ((int) $i) {
             ...
             * */
            $ligne = $db->query('SELECT PSE_EMAILPROF FROM DEPORTSAL d LEFT JOIN SALARIES s ON d.pse_salarie=s.psa_salarie LEFT JOIN RESSOURCE r ON r.ars_salarie=s.psa_salarie WHERE ARS_RESSOURCE="' . $i . '"')->fetchRow();
    ...
    public function getIdProfil() {
        $db = connecter_cegid();
        /*
        return $db->query('SELECT PSA_LIBELLEEMPLOI FROM SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($this->Id_ressource) . '"
                    UNION SELECT PSA_LIBELLEEMPLOI FROM ALCYON.DBO.SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($this->Id_ressource) . '"
                    UNION SELECT PSA_LIBELLEEMPLOI FROM ALTIQUENET.DBO.SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($this->Id_ressource) . '"
                    UNION SELECT PSA_LIBELLEEMPLOI FROM ELITEX.DBO.SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($this->Id_ressource) . '"
                    UNION SELECT PSA_LIBELLEEMPLOI FROM LYNT.DBO.SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($this->Id_ressource) . '"')->fetchRow()->psa_libelleemploi;
         * */
         
        return $db->query('SELECT PSA_LIBELLEEMPLOI FROM SALARIES WHERE PSA_SALARIE="' . self::getCegidEmployeeCode($this->Id_ressource) . '"')->fetchRow()->psa_libelleemploi;
    }
    ...
    public static function getCegidEmployeeCode($i) {
         ...
         * 
         */
        return $db->query('SELECT ARS_SALARIE FROM RESSOURCE WHERE ARS_RESSOURCE="' . $i . '"')->fetchRow()->ars_salarie;
    }
    ...
    public static function getCegidResourceCode($i) {
        if ((is_numeric($i))) {
            ...
            WHERE PSA_NUMEROSS="' . self::getS $db = connecter_cegid();
           ocialInsuranceNumber($i) . '"
     ...
     * @return int
     */
    public function getIdBySocialInsuranceNumber($ss) {
        $db = connecter_cegid();
        $j = 0;
        foreach ($_SESSION['cegid_databases'] as $cegid_database) {
            $requete .= ($j != 0) ? ' UNION' : '';
            $requete .= ' SELECT ARS_RESSOURCE FROM ' . $cegid_database . '.DBO.RESSOURCE
                LEFT OUTER JOIN ' . $cegid_database . '.DBO.SALARIES ON (ARS_SALARIE=PSA_SALARIE AND ARS_TYPERESSOURCE="SAL")
                WHERE PSA_NUMEROSS="' . str_replace(' ', '', $ss) . '"';
            $j++;
        }
        $ligne = $db->query($requete)->fetchRow();
        return $ligne->ars_ressource;
    }
    ...
    public function getAgency() {
        ...
        foreach ($_SESSION['cegid_databases'] as $cegid_database) {
            $requete .= ($j != 0) ? ' UNION' : '';
            $requete .= ' SELECT PSA_ETABLISSEMENT
                          FROM ' . $cegid_database . '.DBO.RESSOURCE 
                          LEFT JOIN ' . $cegid_database . '.DBO.SALARIES AS salaries ON ars_salarie=salaries.psa_salarie 
                          WHERE ARS_RESSOURCE="' . $this->Id_ressource . '"';
            $j++;
        ...
        $ligne = $db->query($requete)->fetchRow();
        return $ligne->psa_etablissement;
    }
    ...
    public static function getContractType($i) {
         ...
         *
         */
        $ligne = $db->query('SELECT PCI_TYPECONTRAT FROM CONTRATTRAVAIL WHERE PCI_SALARIE="' . self::getCegidEmployeeCode($i) . '" AND PCI_DEBUTCONTRAT=(select max(PCI_DEBUTCONTRAT) FROM contrattravail WHERE pci_salarie="' . self::getCegidEmployeeCode($i) . '")')->fetchRow();
    ...
    public function isStaff() {
        ...
        $s = $db->query('SELECT PSA_TRAVAILN1 FROM RESSOURCE LEFT JOIN SALARIES ON ars_salarie=salaries.psa_salarie
                           WHERE ARS_RESSOURCE="' . mysql_real_escape_string($this->Id_ressource) . '"')->fetchRow()->psa_travailn1;
        if($s == '001') {
            return true;
        }
        else {
    ...
    /**
     * Renvoie le mail du gestionnaire paie du salarié
     *
     * @return string
     */
    public function getGestionnairePaieMail() {
        $db = connecter_cegid();
        $s = $db->query('SELECT (SELECT PSE_EMAILPROF FROM DEPORTSAL WHERE RIGHT(PSE_SALARIE, 7) = RIGHT(CC14.CC_ABREGE, 7)) AS MAIL
                FROM SALARIES 
                LEFT OUTER JOIN CHOIXCOD CC14 ON PSA_TRAVAILN2=CC14.CC_CODE AND CC14.CC_TYPE="PST" 
                WHERE CC14.CC_ABREGE IS NOT NULL AND PSA_SALARIE="' . mysql_real_escape_string($this->code_ressource) . '"')->fetchRow()->mail;
        return $s;
    }
    
    /**
     * Renvoie le mail du crh du salarié
     *
     * @return string
     */
    public function getCRHMail() {
        $db = connecter_cegid();
        $s = $db->query('SELECT (SELECT PSE_EMAILPROF FROM DEPORTSAL WHERE RIGHT(PSE_SALARIE, 7) = RIGHT(CC14.CC_ABREGE, 7)) AS MAIL
                FROM SALARIES 
                LEFT OUTER JOIN CHOIXCOD CC14 ON PSA_TRAVAILN4=CC14.CC_CODE AND CC14.CC_TYPE="PSV" 
                WHERE CC14.CC_ABREGE IS NOT NULL AND PSA_SALARIE="' . mysql_real_escape_string($this->code_ressource) . '"')->fetchRow()->mail;
        return $s;
    }
}
