...
class ContratDelegation {
    ...
    /**
     * Id lieux de prestation
     *
     * @access public
     * @var int
     */
    public $id_lieux_prestation;
    ...
    public function __construct($code, $tab) {
        try {
            ...
            elseif (!$code && !empty($tab)) {
                ...
                $this->id_lieux_prestation = $tab['id_lieux_prestation'];
            }
    ...
    public function form() {
        ...
        // Si SLF
            ...
            if ($this->Id_devis != '' && $this->Id_devis != null) {
                ...
                // Lieux de prestation
                $LieuxPrestation = new LieuxPrestation($this->id_lieux_prestation);
                $lieuxPresta = '
                    <select id="lieuxPresta" name="id_lieux_prestation" onChange="updateLieuMission(this.value)">
                        <option value="">Sélectionner un lieu de prestation</option>'.
                        $LieuxPrestation->getList().'
                    </select>
                    <span>Créer un nouveau lieu de prestation</span>';
                ...
                Code OTP :
                    ...
                    <fieldset>
                        ...
                        <span class="infoFormulaire"> * </span> Durée estimée de la mission : <span id="duree_planning"><input type="text" id="duree" name="duree" value="' . $this->duree_mission . '" size="2" /></span> (jours)
                            <br /><br />
                            <span class="infoFormulaire"> * </span> Lieu de Prestation : ' . $lieuxPresta . ' 
                            <span style="padding-left:12px;">Plus de détails :</span> <input type="text" id="lieu_mission" name="lieu_mission" value="' . $this->lieu_mission . '" size="40" />
                            <br /><br />
...
';
    ...
    public function createForm() {
        ...
        $LieuxPrestation = new LieuxPrestation($this->id_lieux_prestation);
        $idLieuPrestation = $LieuxPrestation->getLieuPrestationDetails($LieuxPrestation->id);
		$lieuxPresta = '
                    <select id="lieuxPresta" name="id_lieux_prestation" onChange="updateLieuMission(this.value)">
                        <option value="">Sélectionner un lieu de prestation</option>'.
                        $LieuxPrestation->getList().'
        ...
        $lieuxPresta .= '<br />';
        $lieuxPresta .= '<div id="detailsLieuPrestation" style="padding:12px 12px 12px 12px;">' . $idLieuPrestation . '</div>';
    ...
    public function withoutCaseForm() {
        ...
        <form name="formulaire" enctype="multipart/form-data" action="../com/index.php?a=enregistrerContratDelegationHorsAffaire" method="post">
            ...
            <div class="left">
                ...
                $html .= '<h2>LIEU DE PRESTATION</h2><br />'.
                    $lieuxPresta . '<br /><br />';
            
                
            $html .= '<h2>POSTE DU COLLABORATEUR</h2><br />
...
';
    ...
    public function edit($stock = 0) {
        ...
        if(1 == 1){
            $pdf->MultiCellTag(95, 5, "<t2>Numéro de projet :</t2> <t3>PS01720-XXXXXXXXXXXXXXXXXXXX</t3>", 0, 'L', 0);
        }
        $pdf->MultiCellTag(95, 5, "<t2>Apporteur d'affaire :</t2> <t3>" . $this->apporteur . "</t3>", 0, 'L', 0);

        //if ($affaire->sdm) {
            $pdf->MultiCellTag(95, 5, "<t2>RSDM / SDM / RO :</t2> <t3> Christophe Brohan</t3>", 0, 'L', 0);
        //}
        ...
        if ($this->compte_final != '') {
            ...
            $pdf->MultiCellTag(95, 5, "<t2>RANG 2</t2>", 0, 'L', 0);
        }else{
            $pdf->MultiCellTag(95, 5, "<t2>RANG 1</t2>", 0, 'L', 0);
        }
...
*/
        ...
        $pdf->MultiCellTag(95, 5, "<t2>Lieu de Prestation :</t2> <t3>Lieu de prestation 238</t3>", 0, 'L', 0);
        $pdf->MultiCellTag(95, 5, "<t2>Code Lieu :</t2> <t3>LP236</t3>", 0, 'L', 0);
        $pdf->MultiCellTag(95, 5, "<t2>Type Lieux Prestation :</t2> <t3>Etablissement (Code 10)</t3>", 0, 'L', 0);
        $pdf->MultiCellTag(95, 5, "<t2>Adresse  :</t2> <t3>test ligne adresse 1</t3>", 0, 'L', 0);
        $pdf->MultiCellTag(95, 5, "<t2>Adresse 2 :</t2> <t3>test ligne adresse 2</t3>", 0, 'L', 0);
        $pdf->MultiCellTag(95, 5, "<t2>Commune / INSEE :</t2> <t3>AJOUX - Code Postal : 07000 - INSEE : 07004</t3>", 0, 'L', 0);
        $pdf->MultiCellTag(95, 5, "<t2>Plus de détails :</t2> <t3>GARE SNCF AJOUX</t3>", 0, 'L', 0);
        $pdf->MultiCellTag(95, 5, "<t2>Pole de la mission :</t2> <t3>ASSISTANCE TECHNIQUE</t3>", 0, 'L', 0);
        $pdf->MultiCellTag(95, 5, "<t2>Mode de facturation :</t2> <t3>AT EXTERNE</t3>", 0, 'L', 0);
        $pdf->setY($pdf->GetY() + 1);
...
<br />
- Recrutement Staff <br />
- Imputation : Hors affaire. <br />
- Intitulé de poste : ' . $ressource->profil . ' <br />
- Profil CEGID : ' . $ressource->profil_cegid . ' <br />
- Agence de rattachement : ' . $ressource->agence . ' <br />
    ...
    public function infoRessourceCD($type_ressource = null, $Id_ressource = 0) {
        ...
        if ($type_ressource == 'ST') {
            ...
            */
            ...
            // if($_GET['Type_cd'] == 'HA') { $html .= 'Lieu de Prestation : ' . $lieuxPresta . '<br /><br />'; }
            
            $html .= $ressource->etat . ' <br /><br />
